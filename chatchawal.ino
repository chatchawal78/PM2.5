...
bool servoIsUp = false;  // สถานะปัจจุบัน
...
int beepCount = 0;        // นับจำนวนครั้ง
const int maxBeeps = 10;  // ต้องการ 10 ครั้ง

const unsigned long beepOnTime = 200;   // ติ้ด 0.2 วิ
const unsigned long beepOffTime = 300;  // เว้น 0.3 วิ

// ================= STATE =================
bool dangerMode = false;
bool hasPMData = false;

const unsigned long LINE_INTERVAL = 5UL * 60UL * 1000UL;  // 5 นาที

// ================= DANGER =================
// unsigned long lastDangerNotify = 0;
const unsigned long DANGER_INTERVAL = 5UL * 60UL * 1000UL;  // 5 นาที

// ================= INTERVALS =================
const unsigned long TFT_INTERVAL = 1000;  // 1 วินาที
// const unsigned long LINE_INTERVAL   = 300000;  // 5 นาที
const unsigned long SHEET_INTERVAL = 120000;  // 2 นาที
...
// ===============================
// 3️⃣ ฟังก์ชันวัน / เวลา (วางตรงนี้เลย)
// ===============================
String getDateTH() {
  struct tm timeinfo;
...
// ================= WiFi + LINE =================
// ===== WIFI 1 : บ้าน =====
const char* ssid1 = "CHATCHAWAL_2.4G_AIS";
const char* password1 = "0979560829benz";

// ===== WIFI 2 : Hotspot มือถือ =====
const char* ssid2 = "Benz";
const char* password2 = "0979560829benz";


String LINE_CHANNEL_ACCESS_TOKEN = "mZh2LlUQmbnRieu4ZbEO7LUO0Y8rDcR1jlfl8Ikgu7S4iMnPMxzIYWuV8auB4lJiJFt9D5MhIJZwugSI5HASF6bEn91OyMQ9pk8c/Bi7kEg0vdZmO8csmsHGZKQ1ywsuF25G087KWP8AV69FYO65NwdB04t89/1O/w1cDnyilFU=";
String LINE_USER_ID = "U57669d9304f15c5b6bef8e503bbfb974";


// ================= Timer =================
unsigned long bootTime;
unsigned long lastLineSend = 0;
const unsigned long MIN_LINE_INTERVAL = 60000;  // เว้นอย่างน้อย 1 นาที
unsigned long retryAfter429 = 0;

const unsigned long BOOT_DELAY = 30000;  // 30 วิ หลังบูต
...
bool sendLineMessage(String text) {
  ...
  if (code == 200) {
    lastLineSend = millis();  // บันทึกเวลาที่ส่งล่าสุด
  }
...
String urlEncode(String str) {
  ...
  for (int i = 0; i < str.length(); i++) {
    unsigned char c = str.charAt(i);  // ⭐ แก้ signed bug ตรงนี้
...
void updateBuzzer() {
  ...
  if (buzzerState == false) {
    ...
    // ตอนนี้กำลังดัง → ถึงเวลาให้เงียบ
...
void connectWiFi() {
  ...
  // ⭐ รีเซ็ตอีกครั้งก่อนเปลี่ยนเครือข่าย
...
void setup() {
  ...
  // ⭐ รีเซ็ตตัวกันสแปม ให้ส่งได้ทันที
...
void loop() {
  ...
  if (millis() - lastPMSRead >= 1000) {
    ...
    if (readPMSFrame(buffer)) {
      ...
      aqi = calculateAQI(pm2_5);  // ⭐ ต้องมีทุกครั้ง
  ...
  // 🔴 เข้าโหมดอันตราย (แจ้งครั้งเดียว)
...
//   // ขึ้น
//   servo1.write(120);
//   servo2.write(60);  // กลับด้าน
//   delay(1500);

//   // ลง
//   servo1.write(30);
//   servo2.write(150);  // กลับด้าน
//   delay(1500);
// }